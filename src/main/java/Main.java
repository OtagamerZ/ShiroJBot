...
public class Main extends ListenerAdapter implements JobListener, Job {
    ...
    public void onMessageReactionAdd(MessageReactionAddEvent event) {
        ...
        if (Collections.frequency(message.getReactions(), "\ud83d\udc4d") >= 5) {
            message.pin().queue();
        }
        if (ment.size() > 1) {
    ...
    public void onMessageReceived(MessageReceivedEvent message) {
        if (ready) {
            if (gcMap.get(message.getGuild().getId()).getCanalsug() != null && message.getChannel().getId().equals(gcMap.get(message.getGuild().getId()).getCanalsug())) {
