...
public class Shoukan extends GameInstance<Phase> {
	...
	private boolean debAddCard(Side side, JSONObject args) {
		...
		if (args.getBoolean("is_mod", false) || Account.hasRole(curr.getUid(), false, Role.TESTER)) {
			...
			for (String id : ids.split(",")) {
				CardType type = Bit32.toEnumSet(CardType.class, DAO.queryNative(Integer.class, "SELECT get_type(?1)", id)).stream()
						.findFirst()
	...
	private boolean debUndraw(Side side, JSONObject args) {
		...
		if (args.getBoolean("is_mod", false) || Account.hasRole(curr.getUid(), false, Role.TESTER)) {
			...
			for (int i = 0; i < amount; i++) {
				curr.getMainStack().add(curr.getCards().removeLast(Drawable::isAvailable));
			}
	...
	private boolean flipCard(Side side, JSONObject args) {
		...
		d.setSwitched(true);
		reportEvent("str/flipped_card", true, false, curr.getName(), d, d.getState().toString(getLocale()));
		return true;
	}
	...
	private boolean promoteCard(Side side, JSONObject args) {
		...
		reportEvent("str/promoted_card", true, false, curr.getName(), d);
		return true;
	}
	...
	private boolean sacrificeCard(Side side, JSONObject args) {
		...
		reportEvent("str/sacrificed_card", true, false, curr.getName(), chosen);
		return true;
	}
	...
	private boolean sacrificeBatch(Side side, JSONObject args) {
		...
		reportEvent("str/sacrificed_card", true, false, curr.getName(),
				Utils.properlyJoin(getLocale(), cards.stream().map(Drawable::toString).toList())
		);
		return true;
	}
	...
	private boolean discardCard(Side side, JSONObject args) {
		...
		reportEvent("str/discarded_card", true, false, curr.getName(), d);
		return true;
	}
	...
	private boolean discardBatch(Side side, JSONObject args) {
		...
		reportEvent("str/discarded_card", true, false, curr.getName(),
				Utils.properlyJoin(getLocale(), cards.stream().map(Drawable::toString).toList())
		);
		return true;
	}
	...
	public boolean activateTrap(Senshi trap, EffectParameters ep) {
		...
		Targeting tgt = switch (e.getTargetType()) {
			...
			case ALLY -> {
				if (ep.allies().isEmpty()) {
					yield null;
				}

				yield new Targeting(hand, ep.allies().getFirst().index(), -1);
			}
			case ENEMY -> {
				if (ep.enemies().isEmpty()) {
					yield null;
				}

				yield new Targeting(hand, -1, ep.enemies().getFirst().index());
			}
			case BOTH -> {
				if (ep.allies().isEmpty() || ep.enemies().isEmpty()) {
					yield null;
				}

				yield new Targeting(hand, ep.allies().getFirst().index(), ep.enemies().getFirst().index());
			}
		};
	...
	private ButtonizeHelper getButtons() {
				...
				for (SlotColumn slt : getSlots(curr.getSide())) {
					for (Senshi s : slt.getCards()) {
						if (s != null && s.getIndex() != -1 && s.isBerserk()) {
							List<Senshi> valid = allCards.stream()
									.filter(d -> !d.equals(s))
									.toList();
		...
		helper.addAction(getString("str/view_hand"), w -> {
			...
			BufferedImage img = h.render();
			if (img == null) {
				Objects.requireNonNull(w.getHook())
						.setEphemeral(true)
						.sendMessage(getString("error/empty_hand"))
						.queue();
				return;
			}
			...
			WebhookMessageCreateAction<Message> act = Objects.requireNonNull(w.getHook())
					.setEphemeral(true)
					.sendFiles(FileUpload.fromData(IO.getBytes(img, "png"), "hand.png"));
					...
					if (card instanceof Field) {
						...
						if (passes.isEmpty()) {
							...
							for (int k = 1; k <= passes.size(); k++) {
								int tgt = k;
								TargetType type = passes.get(k - 1);
		...
		helper.addAction(getString("str/view_equips"), w -> Objects.requireNonNull(w.getHook())
				.setEphemeral(true)
		...
		if (getPhase() == Phase.PLAN) {
			...
			if (!curr.getMainStack().isEmpty() && arcade != Arcade.DECK_ROYALE) {
				...
				if (curr.canUseDestiny() && !Utils.equalsAny(curr.getOrigins().major(), Race.MACHINE, Race.MYSTICAL)) {
					helper.addAction(getString("str/draw_destiny"), w -> {
						...
						BondedList<Drawable<?>> deque = curr.getMainStack();
			...
			if (!curr.getDeck().isEmpty() && curr.getOrigins().hasSynergy(Race.ORACLE)) {
	...
	private ButtonizeHelper getExtraButtons(ButtonizeHelper parent, Hand curr) {
		...
		helper.addAction(getString("str/card_skill"), w -> {
			...
			for (SlotColumn slot : slots) {
				Senshi top = slot.getTop();
				if (top == null || !top.isAvailable() || !top.hasAbility()) {
					sourceDisable.add(new Pair<>(slot.getIndex(), false));
				}
			}
			...
			ButtonizeHelper source = makeSelector(curr, 5, 1, (par, j, i) -> {
				...
				if (passes.isEmpty()) {
					...
					List<String> labels = new ArrayList<>();
					labels.add(getString("str/select_source"));

					List<Pair<ButtonizeHelper, List<Pair<Integer, Boolean>>>> pages = new ArrayList<>();
					pages.add(new Pair<>(par, sourceDisable));

					for (int k = 1; k <= passes.size(); k++) {
						int tgt = k;
						TargetType type = passes.get(k - 1);
						List<Pair<Integer, Boolean>> passDisable = new ArrayList<>();
						List<SlotColumn> slts = getSlots(type == TargetType.ENEMY ? curr.getSide().getOther() : curr.getSide());
	...
	public void send(Drawable<?> source, String text, String gif) {
		...
		for (GuildMessageChannel chn : getChannel().getChannels()) {
			PseudoUser pu = new PseudoUser(source.toString(), Constants.API_ROOT + "card/" + source.getCard().getAnime().getId() + "/" + source.getId(), chn);
			if (gif != null) {
