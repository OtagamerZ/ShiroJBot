...
public class Hand {
	...
	public void showHand() {
		...
		g2d.translate(bi.getWidth() / 2f - (164 - cards.size()), 75 + cards.size());

		for (int i = 0; i < cards.size(); i++) {
			KawaiponCard kc = cards.get(i);

			Helper.drawRotated(g2d, kc.getCard().drawCard(kc.isFoil()), 154, 450, -45 + (90f / cards.size() * (i + 1)));
		...
		user.openPrivateChannel().complete().sendFile(Helper.getBytes(bi, "png"), "hand.png").queue();
	}
}
