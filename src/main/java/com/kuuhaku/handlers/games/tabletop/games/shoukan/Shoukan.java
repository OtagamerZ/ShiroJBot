...
public class Shoukan extends GlobalGame implements Serializable {
    ...
    public void attack(Pair<Side, Integer> atkr, Pair<Side, Integer> defr) {
        ...
        if (his.isStasis()) {
            ...
            return;
        } else if (yours.isDuelling() && !yours.getNemesis().equals(his)) {
            channel.sendMessage("❌ | " + yours.getName() + " só pode atacar " + yours.getNemesis() + " (DUELO).").queue();
            return;
        } else if (his.isDuelling() && !his.getNemesis().equals(yours)) {
            channel.sendMessage("❌ | " + his.getName() + " só pode ser atacado por " + his.getNemesis() + " (DUELO).").queue();
            return;
        }
        ...
        /* ATTACK CLASHED */
            ...
            if (h == null || h.getHp() == 0) {
                ...
                int bleed = his.getLinkedTo().parallelStream()
                        .filter(e -> e.getCharm() == Charm.BLEEDING)
                        .mapToInt(Equipment::getAtk)
                        .sum();
            ...
            if (isHero) {
                ...
                int bleed = yours.getLinkedTo().parallelStream()
                        .filter(e -> e.getCharm() == Charm.BLEEDING)
                        .mapToInt(Equipment::getAtk)
                        .sum();
    ...
    public boolean lastTick() {
        for (Side s : Side.values()) {
            ...
            for (int i = 0; i < 5; i++) {
                ...
                applyEffect(h.getHp() <= 0 ? ON_LOSE : ON_WIN, c, i, s, Pair.of(null, -1), Pair.of(null, -1));
            }
    ...
    public void applyEffect(EffectTrigger trigger, Champion activator, int index, Side side, Duelists duelists) {
        applyEffect(trigger, activator, index, side, Pair.of(duelists.getAttacker(), duelists.getAttackerPos()), Pair.of(duelists.getDefender(), duelists.getDefenderPos()));
    }
