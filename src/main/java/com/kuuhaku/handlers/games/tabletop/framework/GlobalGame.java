...
public abstract class GlobalGame {
	...
	public GlobalGame(ShardManager handler, Board board, GameChannel channel) {
	...
	public GlobalGame(ShardManager handler, Board board, GameChannel channel, JSONObject custom) {
		this.handler = handler;
		this.board = board;
		this.channel = channel;
		this.current = handler.getUserById(board.getPlayers().getCurrent().getId());
