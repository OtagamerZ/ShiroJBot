...
@Entity
@Table(name = "evogear")
public class Evogear extends DrawableDAO implements Drawable, Cloneable {
	@Id
	...
	public BufferedImage drawCard(boolean flipped) {
		...
		if (flipped) {
			...
			BufferedImage star = FileHelper.getResourceAsImage(this.getClass(), "shoukan/star.png");
			if (star != null)
			...
			if (linkedTo != null) {
				Champion c = CollectionHelper.getOr(linkedTo.asChampion().getFakeCard(), linkedTo.asChampion());
				BufferedImage linked = linkedTo.asChampion().isFlipped()
						? FileHelper.getResourceAsImage(this.getClass(), "kawaipon/missing.jpg")
	...
	public int getWeight(Deck d, int min) {
		boolean active = LogicHelper.containsAny(getCharms(), Charm.ENCHANTMENT, Charm.TRAP, Charm.SPELL, Charm.CURSE);

		int weight = tier;
		weight += switch (getCharms().size()) {
			case 1, 2 -> {
				if (active && CollectionHelper.getOr(description, "").isBlank()) {
	...
	public String getDescription() {
		return CollectionHelper.getOr(altDescription, description);
	}
	...
	public boolean isSpell() {
		return LogicHelper.containsAny(getCharms(), Charm.SPELL, Charm.CURSE);
	}

	public boolean hasEffect() {
		return CollectionHelper.getOr(altEffect, effect) != null;
	}

	public String getRawEffect() {
		return CollectionHelper.getOr(altEffect, effect);
	}
	...
	public void activate(Hand you, Hand opponent, Shoukan game, int allyPos, int enemyPos) {
		try {
			...
			gs.setVariable("self", this);
			gs.evaluate(CollectionHelper.getOr(altEffect, effect));
		} catch (Exception e) {
			MiscHelper.logger(this.getClass()).warn("Erro ao executar efeito de " + card.getName(), e);
		}
	}
