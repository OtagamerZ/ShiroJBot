...
public class SetKawaiponCommand implements Executable {
	...
	public void execute(JDA bot, I18N locale, EventData data, MessageData.Guild event, JSONObject args) {
		...
		if (args.has("action")) {
			...
			event.channel().sendMessage(locale.get("success/kawaipon_channel_clear")).queue();
			return;
		}
		...
		if (settings.getKawaiponChannels().stream().anyMatch(channel::equals)) {
			settings.getKawaiponChannels().removeIf(channel::equals);
