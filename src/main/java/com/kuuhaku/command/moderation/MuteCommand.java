...
public class MuteCommand implements Executable {
    ...
    public void execute(JDA bot, I18N locale, EventData data, MessageData.Guild event, JSONObject args) {
        ...
        String strDuration = Utils.toStringDuration(locale, duration);

        try {
            Utils.confirm(locale.get("question/mute",
                            members.size() == 1 ? locale.get("str/that_m") : locale.get("str/those_m"),
                            members.size() == 1 ? locale.get("str/user") : locale.get("str/users"),
                            strDuration
                    ), event.channel(), w -> {
                        RestAction.allOf(members.stream().map(m -> m.timeoutFor(duration, TimeUnit.MILLISECONDS).reason(args.getString("reason"))).toList())
                                .flatMap(s -> event.channel().sendMessage(locale.get("success/mute",
                                                s.size(),
                                                members.size() == 1 ? locale.get("str/user") : locale.get("str/users"),
                                                strDuration,
                                                args.get("reason")
