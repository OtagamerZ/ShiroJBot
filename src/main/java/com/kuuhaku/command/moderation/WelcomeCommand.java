...
public class WelcomeCommand implements Executable {
	...
	public void execute(JDA bot, I18N locale, EventData data, MessageData.Guild event, Map<String, String> args) {
		...
		if (args.containsKey("action")) {
			...
			event.channel().sendMessage(locale.get("sucess/welcome_clear")).queue();
			return;
		} else if (args.containsKey("channel")) {
			settings.setChannel(event.guild().getTextChannelById(args.get("channel")));
			settings.save();

			event.channel().sendMessage(locale.get("sucess/welcome_channel_save")).queue();
			return;
		}
		...
		event.channel().sendMessage(locale.get("sucess/welcome_message_save")).queue();
	}
}
