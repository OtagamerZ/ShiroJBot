...
public class VoteCommand extends Command {
	...
	public void execute(User author, Member member, String rawCmd, String[] args, Message message, MessageChannel channel, Guild guild, Event event, String prefix) {
		if (args.length == 0 || message.getMentionedUsers().size() < 1) {
			...
			return;
		} else if (!SQLiteOld.getMemberByMid(author.getId()).canVote()) {
		...
		switch (args[1]) {
			...
			case "pos":
			case ":thumbsup:":
				Votes.voteUser(guild, author, message.getMentionedUsers().get(0), true);
				break;
			case "negativo":
			case "neg":
			case ":thumbsdown:":
				Votes.voteUser(guild, author, message.getMentionedUsers().get(0), false);
				break;
			default:
