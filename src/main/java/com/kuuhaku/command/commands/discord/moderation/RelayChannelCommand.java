...
@Command(
		name = "canalrelay",
		aliases = {"canalrl", "relaychannel"},
		usage = "req_channel-reset",
		category = Category.MODERATION
)
public class RelayChannelCommand implements Executable {

	@Override
	public void execute(User author, Member member, String command, String argsAsText, String[] args, Message message, TextChannel channel, Guild guild, String prefix) {
		GuildConfig gc = GuildDAO.getGuildById(guild.getId());
		if (message.getMentionedChannels().isEmpty() && args.length == 0) {
			TextChannel chn = gc.getRelayChannel();
			if (chn == null)
				channel.sendMessage("Ainda não foi definido um canal global.").queue();
			else
				channel.sendMessage("O canal global atual do servidor é " + chn.getAsMention() + ".").queue();
			return;
		}
