...
public class ShoukanCommand extends Command {
	...
	public void execute(User author, Member member, String rawCmd, String[] args, Message message, MessageChannel channel, Guild guild, String prefix) {
		...
		JSONObject custom;
		if (practice) custom = Helper.getOr(Helper.findJson(rawCmd), new JSONObject());
		else custom = Helper.findJson(rawCmd);

		int bet = 0;
		if (args.length > 1 && StringUtils.isNumeric(args[1]) && custom == null && !practice) {
		...
		if (practice) {
			Game t = new Shoukan(Main.getInfo().getAPI(), (TextChannel) channel, bet, custom, daily, author, author);
			t.start();
		} else {
