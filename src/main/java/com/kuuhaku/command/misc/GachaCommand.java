		...
		try {
			...
			switch (tp) {
				case KAWAIPON -> {
					...
					g2d.drawImage(c.drawCard(kc.isChrome()), 15, 45, null);
					if (kc.isChrome()) {
						g2d.drawImage(IO.getResourceAsImage("kawaipon/frames/" + hPath + "/destiny.png"), 0, 30, null);
					}
				...
				case EVOGEAR -> {
					...
					g2d.drawImage(e.render(locale, deck), 15, 45, null);
					if (e.getTier() == 4) {
						g2d.drawImage(IO.getResourceAsImage("kawaipon/frames/" + hPath + "/hero.png"), 0, 30, null);
					}
				...
				case FIELD -> {
					...
					g2d.drawImage(f.render(locale, deck), 15, 45, null);
					return new StashedCard(kp, c, tp);
				}
			}
		} finally {
