		...
		for (int i = 0; i < phases; i++) {
			...
			for (int k = 0; k < pSize; k++) {
				...
				if (mult == 2) {
					...
					Participant second = ranking.isEmpty() ? null : ranking.lower(ranking.first());
					if (second != null) {
						g2d.setColor(Color.white);
						g2d.setFont(Fonts.HAMMERSMITH_ONE.deriveFont(Font.BOLD, 50 * mult));
					...
					Participant third = second == null ? null : ranking.lower(second);
					if (third != null) {
						g2d.setColor(Color.white);
						g2d.setFont(Fonts.HAMMERSMITH_ONE.deriveFont(Font.BOLD, 50 * mult));
