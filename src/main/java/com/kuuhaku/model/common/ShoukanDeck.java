...
public class ShoukanDeck {
	...
	public BufferedImage view(List<Champion> champs, List<Equipment> equips) throws IOException, InterruptedException {
		...
		for (int i = 0, y = 0; i < champs.size(); i++, y = i / 6) {
			g2d.drawImage(champs.get(i).drawCard(acc, false), 76 + 279 * (i - 6 * y), 349 + 420 * y, null);
			Profile.printCenteredString(StringUtils.abbreviate(champs.get(i).getCard().getName(), 15), 225, 76 + 279 * (i - 6 * y), 769 + 420 * y, g2d);
		}

		for (int i = 0, y = 0; i < equips.size(); i++, y = i / 3) {
			g2d.drawImage(equips.get(i).drawCard(acc, false), 2022 + 279 * (i - 6 * y), 349 + 420 * y, null);
			Profile.printCenteredString(StringUtils.abbreviate(equips.get(i).getCard().getName(), 15), 225, 2022 + 279 * (i - 6 * y), 769 + 420 * y, g2d);
		}
