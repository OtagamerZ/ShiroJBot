...
public class Hand {
	...
	public Hand(String uid, Shoukan game, Side side) {
		...
		if (game.getArcade() == Arcade.CARDMASTER) {
			...
			cards.add(DAO.queryAll(Senshi.class, "SELECT s FROM Senshi s WHERE cast(get_rarity_index(s.card.rarity) AS INTEGER) BETWEEN 1 AND 5 AND NOT has(s.base.tags, 'FUSION')"));
