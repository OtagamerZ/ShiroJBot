...
public class Music {
    ...
    public static void pauseTrack(TextChannel channel) {
        ...
        if (musicManager.player.isPaused()) {
            channel.sendMessage(I18n.getString("err_music-paused")).queue();
            return;
        }
    ...
    public static void resumeTrack(TextChannel channel) {
        ...
        if (!musicManager.player.isPaused()) {
            channel.sendMessage(I18n.getString("err_music-not-paused")).queue();
            return;
        }
    ...
    public static void trackInfo(TextChannel channel) {
        ...
        if (musicManager.player.getPlayingTrack() == null) {
            channel.sendMessage(I18n.getString("err_no-music")).queue();
            return;
        }
    ...
    public static void queueInfo(TextChannel channel) {
        ...
        if (musicManager.player.getPlayingTrack() == null) {
            channel.sendMessage(I18n.getString("err_empty-queue")).queue();
            return;
        }
    ...
    public static void setVolume(TextChannel channel, int volume) {
        ...
        if (volume <= 100 && volume > 0) musicManager.player.setVolume(volume);
            ...
            channel.sendMessage(I18n.getString("err_invalid-volume")).queue();
            return;
        }
        ...
        ShiroInfo.getApm().loadItemOrdered(musicManager, trackUrl, new AudioLoadResultHandler() {
            ...
            public void trackLoaded(AudioTrack track) {
                if (Objects.requireNonNull(m.getVoiceState()).inVoiceChannel()) {
                    ...
                    channel.sendMessage(I18n.getString("err_not-in-voice-channel")).queue();
            }
